...
export async function generateStaticParams() {
  try {
    ...
    return items
      .filter((s) => s.slug.startsWith('devocional-'))
      .map((s) => ({ slug: s.slug.replace(/^devocional-/, '') }));
  } catch {
    return [];
  }
}
...
export async function generateMetadata({ params }) {
  ...
  const slug = `devocional-${slugParam}`;
  let signal;
  try {
...
export default async function DevotionalPage({ params }) {
  ...
  const path = `/devocional/${slugParam}`;
  const intentLabel = INTENT_LABELS[signal.intent] ?? 'Devocionais';
      ...
      <article>
        <header style={{ marginBottom: 'var(--space-5)' }}>
          <div style={{ marginBottom: 'var(--space-3)' }}>
            <a className="tag" href="/devocional" style={{ textDecoration: 'none' }}>
              Devocionais
            </a>
